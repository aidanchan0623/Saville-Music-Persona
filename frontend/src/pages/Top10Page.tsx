  ...
  return (
    <GlowPanel as="article" variant="row" selected={selected} className="p-4 transition" data-testid="top-album-card">
      <div className="grid gap-4 sm:grid-cols-[5rem_1fr] lg:grid-cols-[5rem_1fr_auto]">
        <AlbumCover albumImageUrl={album.album_image_url ?? album.thumbnail} albumTitle={album.album} size="md" fallbackLabel={rank} fit={album.album_image_url ? "cover" : "contain"} />
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-3">
            <span className="text-xl font-black text-red-200">{rank}</span>
          </div>
...
function AlbumDrilldownPanel({ album, response, loading, onClose }: { album: TopAlbumItem; response: TopAlbumSongsResponse | null; loading: boolean; onClose: () => void }) {
  return (
    <DrilldownShell
      ...
      visual={<AlbumCover albumImageUrl={album.album_image_url ?? album.thumbnail} albumTitle={album.album} size="lg" fallbackLabel={initials(album.album)} fit={album.album_image_url ? "cover" : "contain"} />}
      loading={loading}
      onClose={onClose}
