...
import { TrackArtwork } from "../components/Artwork";
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        {groups.map(({ group, items }) => (
          <div key={group} className="border-t border-white/10 py-4">
      ...
      {!recommendations.length ? (
        ...
        <div className="space-y-6">
          {groups.filter((group) => group.items.length).map(({ group, description, items }) => (
            <section key={group} className="min-w-0 border-t border-white/10 pt-5">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                ...
                <span className="text-sm font-semibold text-mist">{items.length} picks</span>
              </div>
  ...
  return (
    <GlowPanel as="article" variant="card" className="min-w-0 p-4 transition">
      <div className="flex gap-4">
        <TrackArtwork trackImageUrl={item.track_image_url} albumArtUrl={item.album_art_url ?? item.album_art} title={item.track_title} size="md" fallbackLabel={`#${item.rank}`} />
        <div className="min-w-0">
          ...
          <h3 className="mt-2 break-words text-lg font-black leading-6 text-white">{item.track_title}</h3>
          <p className="mt-1 text-sm font-semibold leading-5 text-red-100">{item.artist}</p>
          <p className="mt-1 text-sm leading-5 text-mist">{item.album || "Album unavailable"} {item.release_year ? `- ${item.release_year}` : ""}</p>
        </div>
      </div>
      <p className="mt-4 break-words text-sm leading-6 text-mist">{item.why_this_fits}</p>
      {item.musical_connection ? <p className="mt-3 break-words border-t border-white/10 pt-3 text-xs leading-5 text-red-100">{item.musical_connection}</p> : null}
