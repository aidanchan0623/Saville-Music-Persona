...
export function HeroIdentitySection({
  ...
  return (
    <section className="relative isolate min-h-[30rem] overflow-hidden rounded-lg border border-white/10 bg-[#090505] px-6 py-8 shadow-glow md:px-10 lg:px-12">
      ...
      <div className="absolute -right-28 top-10 -z-10 h-96 w-96 rounded-full bg-red-500/20 blur-3xl" />
      ...
      <div className="flex flex-col gap-8 lg:grid lg:grid-cols-[minmax(0,1.35fr)_minmax(18rem,0.65fr)] lg:items-center">
        <div className="max-w-4xl">
          <div className="flex flex-wrap gap-2 text-xs">
            ...
            <span className="rounded-full border border-red-400/25 bg-red-500/10 px-3 py-1 text-red-100">{modelLabel}</span>
          </div>
          <p className="mt-7 text-sm font-semibold uppercase tracking-[0.24em] text-red-100">Private music identity</p>
          ...
          <p className="mt-5 max-w-4xl text-2xl font-black leading-tight text-red-100 md:text-3xl xl:text-4xl">
            {identityTitle}
          </p>
          <p className="mt-5 max-w-3xl text-base leading-8 text-mist md:text-lg">{summary}</p>
          <p className="mt-4 max-w-3xl border-l-2 border-red-500/60 pl-4 text-sm leading-7 text-red-100 md:text-base">
            {currentState}
          </p>
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3 text-sm text-mist">
            ...
            <button className="inline-flex items-center gap-2 text-red-100 transition hover:text-white disabled:opacity-50" onClick={onRefresh} disabled={busy}>
        ...
        <div className="relative mx-auto aspect-square w-full max-w-[22rem] xl:max-w-[24rem]">
          ...
          <div className="absolute inset-[8%] rounded-full border border-red-400/25 bg-red-500/10 shadow-glow" />
          <div className="absolute inset-[21%] rounded-full border border-red-900/35 bg-red-950/30" />
          <div className="absolute inset-[34%] grid place-items-center rounded-full border border-white/15 bg-ink/80">
            <Waves className="text-red-100" size={46} />
          </div>
...
function SoundBar({ className, delay = false }: { className: string; delay?: boolean }) {
  return (
    <span
      className={`absolute w-2 rounded-full bg-gradient-to-t from-red-900 via-red-500 to-red-200 opacity-80 shadow-[0_0_24px_rgba(239,68,68,0.45)] ${className} ${delay ? "animate-[pulse_3s_ease-in-out_infinite]" : "animate-[pulse_4s_ease-in-out_infinite]"}`}
    />
  );
}
