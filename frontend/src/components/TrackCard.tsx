...
export function TrackCard({ track }: { track: TopTrack }) {
  return (
    <article className="group grid grid-cols-[3rem_4.5rem_1fr] gap-4 rounded-lg border border-line bg-panel/80 p-4 transition hover:border-red-400/40 hover:bg-panelSoft/85">
      ...
      <div className="h-[4.5rem] w-[4.5rem] overflow-hidden rounded-md bg-white/10">
        {track.thumbnail ? <img className="h-full w-full object-cover" src={track.thumbnail} alt="" /> : <div className="h-full w-full bg-red-500/20" />}
      </div>
      <div className="min-w-0">
        <h3 className="truncate text-lg font-semibold text-white">{track.title}</h3>
        <p className="mt-1 truncate text-sm text-red-100">{track.artist}</p>
