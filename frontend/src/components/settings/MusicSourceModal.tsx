...
export function MusicSourceModal({ open, onClose, onConnectSpotify, onImportTakeout, onImportSpotifyHistory, busy, message, spotifyConfigured }: Props) {
    ...
    <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-black/75 p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="music-source-title" className="w-full max-w-4xl rounded-xl border border-red-500/25 bg-[#101012] p-5 shadow-2xl md:p-7">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          ...
          <section className="rounded-lg border border-white/10 bg-black/25 p-5"><h3 className="text-xl font-black text-white">Spotify</h3><p className="mt-2 text-sm leading-6 text-mist">Upload your Spotify extended streaming-history ZIP for dated plays. OAuth is optional and adds catalogue metadata such as images, albums, and artist genres.</p><div className="mt-4 flex flex-wrap gap-3"><label className="btn-primary inline-flex cursor-pointer"><Upload size={16}/> Upload Spotify Export<input className="sr-only" disabled={busy} type="file" accept=".json,.zip,application/json,application/zip" onChange={(event) => { void uploadSpotify(event.target.files?.[0]); event.currentTarget.value = ""; }} /></label><button type="button" className="btn-secondary" disabled={busy || !spotifyConfigured} onClick={onConnectSpotify}>Connect Spotify</button></div><button type="button" className="btn-secondary mt-3" onClick={() => openExternal("https://www.spotify.com/account/privacy/")}><ExternalLink size={16}/> Open Spotify Data Download</button><p className="mt-4 text-sm leading-6 text-mist">Saville accepts the downloaded ZIP directly, reads only audio history JSON, excludes podcasts and audiobooks, and safely deduplicates overlapping export files.</p></section>
        </div>
