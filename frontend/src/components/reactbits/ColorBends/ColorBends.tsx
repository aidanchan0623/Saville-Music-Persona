...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    renderer.setClearColor(0x030203, transparent ? 0 : 1);
    ...
    const loop = () => {
      ...
      const degrees = (rotationRef.current % 360) + (reducedMotion ? 0 : autoRotateRef.current * elapsed);
  ...
  useEffect(() => {
    ...
    material.uniforms.uIterations.value = Math.min(5, Math.max(1, Math.round(iterations)));
    material.uniforms.uIntensity.value = intensity;
    material.uniforms.uBandWidth.value = bandWidth;
    const toVector = (hex: string) => {
      const color = new THREE.Color(hex);
      return new THREE.Vector3(color.r, color.g, color.b);
    };
    const palette = colors.filter(Boolean).slice(0, MAX_COLORS).map(toVector);
    const uniformColors = material.uniforms.uColors.value as THREE.Vector3[];
    for (let index = 0; index < MAX_COLORS; index += 1) {
      if (index < palette.length) uniformColors[index].copy(palette[index]);
      else uniformColors[index].set(0, 0, 0);
    }
  ...
  useEffect(() => {
    const container = containerRef.current;
    if (!container || (mouseInfluence === 0 && parallax === 0)) return;
